...
train_data_iter = iter(train_data) # 50개씩 들고올 수 있는 iterator 지정

# tf.keras.Model을 이용해서 CNN 모델을 정의합니다.
class CNN(tf.keras.Model):
  def __init__(self):
		# 상속 받은 keras model 생성자 부분 호출
    super(CNN, self).__init__()
    # 첫번째 Convolution Layer
    # 5x5 Kernel Size를 가진 32개의 Filter를 적용합니다.
		# convolution
		# padding -> same / valid
		# same : 원본 사이즈와 동일한 사이즈가 되도록 제로 패딩을 넣어줌
		# valid : 계산 그대로 출력
    ...
    self.flatten_layer = tf.keras.layers.Flatten() # flattening
  ...
  def call(self, x):
		# CNN은 인풋 데이터를 flattening 된 vector로 받을 필요 x
    ...
    # 14x14x64 -> 7x7x64 (pooling 시점에서 size 변경)
    h_pool2 = self.pool_layer_2(h_conv2)
    # 7x7x64(3136) -> 1024 (하나의 긴 3136 flattening vector -> 1024 노드 개수를 가진 fully connected layer로 넘김
    h_pool2_flat = self.flatten_layer(h_pool2)
    h_fc1 = self.fc_layer_1(h_pool2_flat)
    # 1024 -> 10 (10차원 softmax regression vector로 변경)
    logits = self.output_layer(h_fc1)
    y_pred = tf.nn.softmax(logits)

		# return1 : softmax function을 씌운 normalize 된 output
		# return2 : softmax function을 씌우기 전 로직 값
